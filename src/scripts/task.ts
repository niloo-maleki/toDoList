...
export class Task {
  ...
  addRow(task: ITask, index: number) {
    ...
    <select  onchange="changePositionHandler()" class="border-gray-200 px-5 py-3b bg-white">
     ${states.map(
       (item) =>
         `<option ${item === task.status ? "selected" : ""}>${item}</option>`
     )}
      </select>
    </td>
    <td class="border-b border-gray-200 bg-white text-sm"><button data-id=${index} class="delete"><svg width="24" height="28" viewBox="0 0 24 28" fill="none" xmlns="http://www.w3.org/2000/svg">
